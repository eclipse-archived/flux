...
public class CloudFoundryClientDelegate {
	...
	protected void handleMessage(String message, String streamType,
			String appName) throws Error {
		if (connector == null) {
			throw new Error(message);
		}
		JSONObject message = new JSONObject();
		message.put(MessageConstants.USERNAME, this.cfUser);
		message.put(MessageConstants.CF_APP, appName);

		if (this.orgSpace != null) {
			String[] pieces = getOrgSpace(this.orgSpace);
			String org = pieces[0];
			String space = pieces[1];
			message.put(MessageConstants.CF_ORG, org);
			message.put(MessageConstants.CF_ORG_SPACE, space);
		}
		message.put(MessageConstants.CF_MESSAGE, message);
		message.put(MessageConstants.CF_STREAM, streamType);

		connector.send(MessageConstants.CF_APP_LOG, message);
